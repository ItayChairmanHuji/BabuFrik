...
def build_task(task_config: TaskConfiguration) -> Task:
    ...
    fd_path = __get_fd_path(task_config)
    marginals_path = __get_marginals_path(task_config)
    jobs = [job_builder.build_job(service, task_config, run) for service in task_config.services]
    return Task(task_config.working_dir, fd_path, marginals_path, jobs)
