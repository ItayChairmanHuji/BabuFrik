...
class VertexCoverRepairer(Node):
    ...
    def __generate_model(self) -> gp.Model:
        ...
        return gp.Model("VertexCover", env=env)
    ...
    def __find_tuple_cost(self, data: DataFrame, tuple_index: int, marginals: MarginalsCalculator.Marginals) -> float:
        synthetic_marginals = MarginalsCalculator.calculate_marginal(data.drop(index=tuple_index))
        marg_diff = sum(
            (priv_attr_marg["size"] - synth_attr_marg["size"]).sum()
            for priv_attr_marg, synth_attr_marg in zip(marginals.values(), synthetic_marginals.values())
        )
        marg_count = sum(
            (priv_attr_marg - synth_attr_marg).count()
            for priv_attr_marg, synth_attr_marg in zip(marginals.values(), synthetic_marginals.values())
        )
        return marg_diff/marg_count
