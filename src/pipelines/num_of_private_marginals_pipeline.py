...
class NumOfPrivateMarginalsPipeline(Pipeline):
    def create_initial_tasks(self) -> list[Task]:
        return [Task(
            ...
            fds=self.fds,
            relative_num_of_private_marginals=relative_num_of_marginals,
            marginals_privacy_budget=self.config.marginals_privacy_budget)
            for relative_num_of_marginals in self.config.relative_num_of_marginals]
