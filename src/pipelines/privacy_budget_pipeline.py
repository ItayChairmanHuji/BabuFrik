...
class PrivacyBudgetPipeline(Pipeline):
    def create_initial_tasks(self) -> list[Task]:
        return [Task(
            ...
            marginals_privacy_budget=marginals_privacy_budget)
            for marginals_privacy_budget in self.config.marginals_privacy_budget]
